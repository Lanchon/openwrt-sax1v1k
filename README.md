...
- It only writes to the last sector of the recovery OS partition, and automatically recovers from eventual corruption in that sector.
...
- 3x 1-second flashes: early boot.
...
- More flashing as the device continues to boot.
...
Any non-interrupted boot will reset the count. After 3 consecutive boot interrupts, the next boot will skip the 5-second LED ON stage and boot to recovery or via TFTP.
...
4. Once logged in, use a terminal command to send the raw `configure-uboot.sh` file; the script will start executing when fully transferred.
   ...
   (Or you can copy it through the terminal to a file inside `/tmp`, `chmod +x` it, and execute the script locally.)
...
9. Reboot the router. You should see it failing to boot the main OS, then falling back to the recovery OS and succeeding.
   ...
   Go to `System`/`Backup / Flash Firmware` and hit `Flash image...` to flash an OpenWrt sysupgrade image; choose to wipe settings.
10. Reboot the router and verify that it boots the main OS successfully.

### U-Boot Configuration Upgrade

You can also run this script under official OpenWrt firmware at anytime, to upgrade the U-Boot configuration of the router to the latest version.
...
(Or you could transfer the file to the router and run it locally, of course.)
...
Follow the steps in the next section in that case.
...
3. Type `blkdiscard -f /dev/mmcblk0p36`.
   (If you do not have the `blkdiscard` command on the router, just ignore the failure and move on; this is a cleanup command that is not really necessary.)
